...
# PyTorch 2.6+: torch.load defaults to weights_only=True, which restricts unpickling.
# Older checkpoints may reference argparse.Namespace; allowlist it safely to avoid errors.
try:
    from torch.serialization import add_safe_globals as _add_safe_globals
    import argparse as _argparse
    _add_safe_globals([_argparse.Namespace])
except Exception:
    # If running on older PyTorch without add_safe_globals, just proceed.
    pass
        ...
        for idx,res in enumerate(value):
            with torch.no_grad():
                ...
                if args.fp16 and device.type == 'cuda':
                    z = torch.randn(1, args.volume_channels, res[0], res[1],res[2], device=device, dtype=torch.float16)
                else:
                ...
                if args.fp16 and device.type == 'cuda':
                    # Optional: convert model & AE to half to further reduce memory
                    model_half = getattr(main, "_model_half_applied", False)
                    if not model_half:
                        model.half()
                        AE.half()
                        setattr(main, "_model_half_applied", True)
                    with torch.autocast(device_type='cuda', dtype=torch.float16):
                        samples = diffusion.sample(
                            model, z, y = y, res=res_emb, strategy = args.sampling_strategy
                        )
                        samples = samples.to(AE.codebook.embeddings.dtype)
                ...
                volume = volume.detach().squeeze(0).float().cpu()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fp16", action='store_true', help="Enable FP16 autocast to reduce memory usage")
